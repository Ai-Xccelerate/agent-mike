...
describe("crypto lazy ENCRYPTION_KEY", () => {
  it("imports without throwing when ENCRYPTION_KEY is unset", async () => {
    ...
    vi.resetModules();
    await expect(import("@/lib/tools-integrations/crypto")).resolves.toMatchObject({
  ...
  it("encrypt throws when ENCRYPTION_KEY is unset", async () => {
    delete process.env.ENCRYPTION_KEY;
    vi.resetModules();
    const { encrypt } = await import("@/lib/tools-integrations/crypto");
  ...
  it("decrypt throws when ENCRYPTION_KEY is unset", async () => {
    delete process.env.ENCRYPTION_KEY;
    vi.resetModules();
    const { decrypt } = await import("@/lib/tools-integrations/crypto");
