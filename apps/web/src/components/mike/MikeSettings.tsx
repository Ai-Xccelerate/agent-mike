        ...
        <div className="min-w-0 flex-1 space-y-5 md:space-y-6">
          <section id="identity" onMouseEnter={() => setActive("identity")} className={cardClass}>
            ...
            <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300 sm:col-span-2">AgentMail address<div className="relative mt-2"><EnvelopeIcon className="absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-gray-400" /><input value={profile.email} onChange={(event) => update("email", event.target.value)} className={`${fieldClass} pl-10`} /></div><span className="mt-1.5 block text-xs font-normal text-gray-500">Use an AgentMail inbox ID or your verified custom domain.</span></label>
