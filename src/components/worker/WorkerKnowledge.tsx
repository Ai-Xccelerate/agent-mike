...
import { apiFetch, KnowledgeDocument } from "@/lib/worker-api";
...
export default function WorkerKnowledge() {
  ...
  useEffect(() => {
    apiFetch<KnowledgeDocument[]>("/knowledge").then(setDocuments).catch(() => undefined);
  }, []);

  const visible = useMemo(
    () =>
      documents.filter((document) =>
        `${document.title} ${document.description ?? ""} ${document.tags.join(" ")}`.toLowerCase().includes(query.toLowerCase()),
      ),
    [documents, query],
  );
  ...
  async function upload() {
    ...
    try {
      ...
      const results = await apiFetch<IngestResult[]>("/knowledge/ingest-files", { method: "POST", body: form });
      ...
      setNotice(
        failed.length
          ...
          : `${ingested.length} ${ingested.length === 1 ? "document is" : "documents are"} ready.`,
      );
    } catch (error) {
      setNoticeError(true);
  ...
  return (
    <div className="space-y-5 md:space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="font-display text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">Knowledge</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Approved reference material this worker can cite in replies.</p>
        </div>
        <Badge size="sm" color="success" startIcon={<CheckCircleIcon className="size-3.5" />}>
          {documents.length} concepts ready
        </Badge>
      </div>
      ...
      <section className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_360px] md:gap-6">
        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
          <div className="flex flex-col gap-3 border-b border-gray-200 p-4 dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between md:px-6">
            <div>
              <h2 className="text-base font-semibold text-gray-800 dark:text-white/90">Knowledge library</h2>
              <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">OKF concept documents, indexed with Postgres full-text search</p>
            </div>
            <input
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="Search knowledge…"
              className="h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3 text-sm text-gray-800 outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/10 dark:border-gray-700 dark:text-white/90 sm:w-60"
            />
          </div>
          <div className="divide-y divide-gray-100 dark:divide-gray-800">
            {visible.map((document) => (
              <article key={document.id} className="flex flex-col gap-4 px-4 py-4 sm:flex-row sm:items-center md:px-6">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="text-sm font-semibold text-gray-800 dark:text-white/90">{document.title}</h3>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="flex size-8 items-center justify-center rounded-lg text-gray-400 hover:bg-error-50 hover:text-error-600 dark:hover:bg-error-500/10"
                  >
            ...
            {!visible.length && (
              <div className="px-6 py-12 text-center text-sm text-gray-500">
                {query ? "No knowledge matches your search." : "No knowledge yet. Upload a file to get started."}
              </div>
            )}
          </div>
        </div>
        ...
        <aside className="space-y-5">
          <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
            ...
            <p className="mt-1 text-sm leading-6 text-gray-500 dark:text-gray-400">
              Upload one or more PDF, Markdown, or text files. PDFs and plain text are auto-wrapped; Markdown with OKF frontmatter is ingested as-is.
            </p>
            <div className="mt-4">
              <FileInput multiple accept=".pdf,.md,.markdown,.txt,.text" onChange={(event) => setFiles(Array.from(event.target.files || []))} />
            </div>
            {files.length > 0 && (
              <ul className="mt-2 space-y-1">
                {files.map((item) => (
                  <li key={item.name} className="truncate text-xs text-gray-500">
                    {item.name}
                  </li>
                ))}
              </ul>
            ...
            <Button className="mt-4 w-full" loading={uploading} disabled={!files.length} onClick={upload}>
              {files.length > 1 ? `Validate and ingest ${files.length} files` : "Validate and ingest"}
            </Button>
          </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
            ...
            <p className="mt-3 text-xs leading-5 text-gray-500 dark:text-gray-400">
              Keep one concept per file. Use headings for better chunks.
            </p>
          </div>
        </aside>
      </section>
