  ...
  return (
    <div className="relative">
      ...
      <Dropdown isOpen={open} onClose={() => setOpen(false)} className="absolute right-0 mt-[17px] w-[calc(100vw-2rem)] max-w-[360px] rounded-2xl border border-gray-200 p-3 shadow-theme-lg dark:border-gray-800 sm:w-[360px]">
        <div className="flex items-center justify-between border-b border-gray-100 px-1 pb-3 dark:border-gray-800">
          <h2 className="text-base font-semibold text-gray-800 dark:text-white/90">Needs your attention</h2>
          <Badge size="sm" color={items.length ? "warning" : "light"}>
            {items.length} open
          </Badge>
        </div>
        <div className="mt-2 space-y-1">
          ...
          {items.map((conversation) => (
            <Link key={conversation.id} href="/inbox" onClick={() => setOpen(false)} className="flex gap-3 rounded-lg p-3 hover:bg-gray-50 dark:hover:bg-white/5">
              <span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-warning-50 text-xs font-semibold text-warning-700 dark:bg-warning-500/10 dark:text-warning-400">
        ...
        <Link href="/inbox" onClick={() => setOpen(false)} className="mt-2 block rounded-lg border border-gray-200 px-3 py-2 text-center text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-800 dark:text-gray-300 dark:hover:bg-white/5">
          Open inbox
        </Link>
      </Dropdown>
    </div>
