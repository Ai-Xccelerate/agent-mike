...
export default function NoAccessScreen({ reason }: { reason?: string }) {
  const manageUrl = CORE_APP ? `${CORE_APP}/dashboard/agents` : "";
    ...
    <div className="flex min-h-screen items-center justify-center p-8">
      <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
        <h1 className="mb-2 text-lg font-semibold text-gray-900 dark:text-white">
          No access to Agent Mike
        </h1>
        <p className="mb-6 text-sm leading-relaxed text-gray-500 dark:text-gray-400">
          {reason ??
            "Your organization does not have Mike enabled, or your account is not assigned. Ask an admin to enable Mike in AIX Core."}
        </p>
        {manageUrl ? (
          <a
            href={manageUrl}
